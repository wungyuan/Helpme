...
export default function SharePoster({ title, shareUrl, rewardType, asOriginator }: Props) {
  ...
  useEffect(() => {
    ...
    async function draw() {
      ...
      const lead = asOriginator ? '🙏 想麻烦你搭把手' : '🤝 接力一下，这事可能就成了';
      ...
      const askLines = wrapByChar(measure, ask, contentW);
      ...
      ctx.fillText('长按二维码「识别图中二维码」，帮我接一棒', W / 2, y);
  ...
  }, [title, shareUrl, rewardType, asOriginator]);
